...
- observability notes
- local mock-data workflow for fast homepage/UI iteration<<<<<<< codex/revise-codex-spec-for-radius-external-skills-sa7hew
...
| `RADIUS_SKILLS_BRANCH` | Default branch when webhook/manual sync does not provide an explicit ref override. Defaults to `main`. |
| `RADIUS_SKILLS_ALLOWED_BRANCHES` | Optional comma-separated branch patterns (e.g. `main,feature/*`) accepted from webhook refs. |
| `RADIUS_SKILLS_ACCEPT_ANY_BRANCH` | When `true`, accept any branch ref from trusted repo webhooks. Defaults to `false`. |
...
1. Set:
   ...
   - optional: `RADIUS_SKILLS_REPO`, `RADIUS_SKILLS_BRANCH`, `RADIUS_SKILLS_ALLOWED_BRANCHES`, `RADIUS_SKILLS_ACCEPT_ANY_BRANCH`, `RADIUS_SKILLS_GITHUB_TOKEN`
...
3. Use internal observability endpoints:
   ...
   - optional manual refresh: `POST /internal/skills/sync` with `{"after":"<commit-sha>","ref":"refs/heads/feature/my-branch"}`.

Webhook trust/sync behavior:
- `RADIUS_SKILLS_REPO` is the trust boundary (repo must match).
- Branch policy is controlled by `RADIUS_SKILLS_ACCEPT_ANY_BRANCH` or `RADIUS_SKILLS_ALLOWED_BRANCHES`.
- `RADIUS_SKILLS_BRANCH` is the default ref fallback (not a hard required incoming ref).
- Webhook payload can include override fields (`sync_ref`, `sync_sha`) at top-level or under `hermes` for explicit sync target selection.
