...
The agent runs `/app/scripts/radius/cmd-wallets.mjs` (including `--set-default=...`), `cmd-balance.mjs`, `cmd-send.mjs` (supports `--asset=sbc|rusd`), and `cmd-fund.mjs` (`--wallet=all` plus backward-compatible `both`).


### Para build-assist pattern

Para docs include an AI-agent setup prompt. If you use Cursor/Claude Code/Codex-style agents, you can run:

```text
Fetch https://docs.getpara.com/skill.md and help me build with Para
```

The onboarding CLI prints this prompt when Para wallet setup is selected.
